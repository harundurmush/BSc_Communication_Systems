...
for i = 1:length(Eb_N0_dB)
    ...
    for k = 1:nTAP
        ...
        %%
         channel_matrix_zf = toeplitz([channel_response(2:end) zeros(1,2*k+1-L+1)], [ channel_response(2:-1:1) zeros(1,2*k+1-L+1) ]);
         d_zf  = zeros(1,2*k+1);
         d_zf(k+1) = 1;
         channel_zf  = (inv(channel_matrix_zf)*d_zf.').';
         %%  matched filter
        ...
        %%
         ...
         channel_matrix_mmse = toeplitz([hAutoCorr([3:end]) zeros(1,2*k+1-L)], [ hAutoCorr([3:end]) zeros(1,2*k+1-L) ]);
         channel_matrix_mmse = channel_matrix_mmse + 1/2*10^(-Eb_N0_dB(i)/10)*eye(2*k+1);
         d_mmse  = zeros(1,2*k+1);
         d_mmse([-1:1]+k+1) = fliplr(channel_response);
         channel_mmse  = [inv(channel_matrix_mmse)*d_mmse.'].';
         %%  matched filter
...
figure (1)
% subplot(211)
semilogy(Eb_N0_dB,theoryBer(1,:),'s-','LineWidth',1.5);
hold on
semilogy(Eb_N0_dB,simBer_zf(1,:),'-.','LineWidth',1.5);
semilogy(Eb_N0_dB,simBer_zf(2,:),'-.','LineWidth',1.5);
% semilogy(Eb_N0_dB,simBer_zf(4,:),'-.');
% semilogy(Eb_N0_dB,simBer_mmse(1,:),'s-');
semilogy(Eb_N0_dB,simBer_mmse(1,:),'o-','LineWidth',1.5);
semilogy(Eb_N0_dB,simBer_mmse(2,:),'o-','LineWidth',1.5);
% semilogy(Eb_N0_dB,simBer_mmse(3,:),'s-');
% semilogy(Eb_N0_dB,simBer_mmse(4,:),'s-');
% axis([0 10 10^-3 0.5])
grid on
legend('theoritical response','3-tap zf','5-tap zf','3-tap mmse','5-tap mmse');
